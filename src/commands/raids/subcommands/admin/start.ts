...
const bossOptions = Array.from({ length: validBosses.length }).map(
  (_, idx) => ({
    name: `boss_${idx + 1}`,
    ...
    return {
      ...
      callback: async (client, interaction) => {
        try {
          ...
          const role_req = await guild.roles.fetch(raidRole, { force: true });
          ...
          if (!reviewTimers.has(raidMsg.id)) {
            ...
            setTimeout(async () => {
              ...
              const role = await guild.roles.fetch(participantRole, {
                force: true,
              });
              ...
              const removeRole = async (userID: string) => {
                try {
                  ...
                  console.error(
                    "Error removing particiaption role after raid has been finished : ",
                    err
                  );
                }
              };
