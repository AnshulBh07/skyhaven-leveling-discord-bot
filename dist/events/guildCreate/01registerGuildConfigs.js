...
const execute = (client, guild) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const freshGuild = yield client.guilds.fetch({
            guild: guildId,
            force: true,
        });
        const guild_members = Array.from(freshGuild.members.cache.entries()).map(([_, member]) => member.user);
        if (!guild || !client.user || existingGuild)
            return;
        // create level roles
