            ...
            callback: (client, interaction) => __awaiter(void 0, void 0, void 0, function* () {
                try {
                    ...
                    if (adminCommands.includes(subcommandName)) {
                        if (!(yield (0, permissionsCheck_1.isManager)(client, interaction.user.id, guild.id, "raid"))) {
                            yield interaction.editReply({
                                content: "⚠️ You lack the required permissions to use this command.",
                            });
                            return;
                    ...
                    if (userCommands.includes(subcommandName)) {
                        if (!(yield (0, permissionsCheck_1.isUser)(client, interaction.user.id, guild.id, "raid"))) {
                            yield interaction.editReply({
                                content: "⚠️ You lack the required permissions to use this command.",
                            });
                            return;
                    ...
                    if (!botAdminIDs.includes(interaction.user.id) &&
                        channel.id !== raidChannelID) {
                        yield interaction.editReply({
                            content: `⚠️ You cannot use this command in this channel. Please use it in <#${raidChannelID}>.`,
                        });
                        return;
                    }
                    // call the function
