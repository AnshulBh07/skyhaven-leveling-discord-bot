...
const getLuminance_1 = require("./getLuminance");
const adjustHexShades_1 = require("./adjustHexShades");
const generateBackground = (bgColor, baseColor) => {
    const canvas = (0, canvas_1.createCanvas)(800, 180);
    const ctx = canvas.getContext("2d");
    const luminance = (0, getLuminance_1.getLuminance)(baseColor);
    const percOffset = 10;
    const percentagesArr = [50, 60, 70, 75, 80, 95, 40];
    const bubblesColors = (luminance < 0.5
        ? percentagesArr
        : percentagesArr.map((perc) => perc + percOffset)).map((perc) => (0, adjustHexShades_1.adjustHexColorBrightness)(baseColor, perc));
    // base for card
    ctx.globalAlpha = 1;
