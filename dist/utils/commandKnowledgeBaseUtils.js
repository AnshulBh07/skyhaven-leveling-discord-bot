...
exports.matchCommand = exports.getIntentScore = void 0;
const fuse_js_1 = __importDefault(require("fuse.js"));
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const getIntentScore = (query) => {
    ...
    if (lower.startsWith("what does"))
        score += 3;
    if (lower.includes("usage") || lower.includes("use"))
        score += 2;
    if (lower.includes("submit") || lower.includes("start"))
        score += 3;
    if (lower.includes("delete") || lower.includes("remove"))
        score += 2;
    if (lower.includes("command") || lower.includes("cmd"))
        score += 3;
    if (lower.includes("where can i"))
        score += 2;
    if (lower.includes("show me") ||
        lower.includes("check") ||
        lower.includes("can i"))
        score += 2;
    if (lower.includes("see my") || lower.includes("view my"))
        score += 2;
    // Mentions of bot name (encourages intent)
    if (lower.includes("seraphina") || lower.includes("you"))
        score += 1;
    ...
    if (lower.includes("maze") ||
        ...
        score += 1;
    if (lower.includes("leaderboard") || lower.includes("board"))
        score += 1;
...
exports.getIntentScore = getIntentScore;
const commands = JSON.parse(fs_1.default.readFileSync(path_1.default.join(__dirname, "../data/commandKnowledgeBase.json"), "utf-8"));
const commandFuse = new fuse_js_1.default(commands, {
    keys: [
